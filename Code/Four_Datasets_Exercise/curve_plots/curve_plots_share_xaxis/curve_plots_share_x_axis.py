...
axs[0].set_yticks(np.arange(0.4, 1.7, 0.2))
...
axs[1].set_yticks(np.arange(0.3, 1.0, 0.1))
...
for i, (region_name, region_coords) in enumerate(regions.items()):
    ...
    axs[0].set_title(f'AOD Area-Averaged Time Series in {first_region} (Top Graph)\nAOD Area-Averaged Time Series in {second_region} (Bottom Graph)', fontsize=8)
